...
엔터프라이즈 AI 도입은 기존 솔루션으로는 해결되지 않는 근본적인 장벽에 직면합니다:

```mermaid
graph LR
    subgraph challenges [엔터프라이즈 AI의 도전과제]
        A[데이터 사일로]
        B[통합 복잡성]
        C[관측성 부재]
        D[배포 경직성]
    ...
    subgraph solutions [Agent Portal 솔루션]
        A1[통합 데이터 접근]
        B1[MCP 표준 프로토콜]
        C1[완전한 AI 관측성]
        D1[컴포저블 아키텍처]
...
| 도전 과제 | 업계 현실 | Agent Portal 솔루션 |
|-----------|----------|----------------------|
| **데이터 사일로** | 기업 데이터의 73%가 AI 시스템에서 접근 불가 | **통합 데이터 접근** — 데이터 이동 없이 모든 소스에 연결 |
| **통합 복잡성** | 평균 기업이 130개 이상의 SaaS를 사용, 각각 커스텀 통합 필요 | **MCP 프로토콜** — 모든 에이전트-도구 연결을 위한 단일 표준 |
| **관측성 부재** | 대부분의 조직이 에이전트 레벨에서 AI 비용 추적 불가 | **완전한 관측성** — 에이전트별 실시간 비용, 지연, 토큰 추적 |
| **배포 경직성** | 클라우드 전용 또는 온프렘 전용 솔루션이 기업 유연성 제한 | **컴포저블 아키텍처** — 어디서든 배포, 무엇이든 스케일 |
| **에이전트 고립** | 에이전트들이 도구로서 협업할 표준 프로토콜 부재 | **멀티에이전트 오케스트레이션** — 에이전트가 다른 에이전트의 도구가 됨 |

---

## 우리의 철학

> *"MCP는 AI 시스템을 데이터 소스와 연결하는 범용적이고 개방된 표준을 제공하며, 파편화된 통합을 단일 프로토콜로 대체합니다."*  
> — [Anthropic, Model Context Protocol](https://modelcontextprotocol.io/)

### 🔗 데이터 이동 없이 통합 접근

데이터베이스, API, MCP 서버, 레거시 시스템 등 모든 데이터 소스에 **데이터를 복사하거나 이동하지 않고** 연결합니다. 사용자는 데이터가 어디 있는지가 아니라 권한으로 정보에 접근합니다. 데이터는 제자리에 있고, 에이전트가 찾아갑니다.

### 💰 비용 인지형 지능

모든 에이전트 호출은 관측 가능합니다. 단순한 작업은 효율적인 모델에, 복잡한 추론은 고성능 모델에 라우팅합니다. **실시간 비용과 성능 메트릭으로 정보에 기반한 의사결정을 내리세요.** 더 이상 깜짝 청구서나 블랙박스 AI 지출이 없습니다.

### 🧩 모놀리식이 아닌 컴포저블

모든 컴포넌트—에이전트, 데이터 커넥터, 워크플로우, 모니터링—가 **독립적으로 동작**합니다. 온프레미스, 클라우드, 또는 하이브리드로 배포하세요. 필요한 것만 스케일하고, 원하는 것을 교체하세요. 벤더 종속 없이, 전부-아니면-전무 결정 없이.

### 🤖 도구로서의 에이전트

에이전트는 단순히 실행되는 것이 아니라 **다른 에이전트의 도구가 됩니다**. 특화된 에이전트들이 자율적으로 협업하는 정교한 멀티에이전트 오케스트레이션을 구축하세요. 재무 분석 에이전트가 데이터 수집 에이전트를 호출하고, 그 에이전트는 다시 여러 데이터베이스를 조회합니다.

### 🌐 오픈 스탠다드 우선

**MCP** (Anthropic), **OpenTelemetry** (CNCF), **LiteLLM** (100+ 모델)을 기반으로 구축되었습니다. 독점 프로토콜 없음, 벤더 종속 없음. Agent Portal에 대한 투자는 개방적이고 이식 가능한 AI 인프라에 대한 투자입니다.
...
- **🔍 기업 공시** — "현대자동차 최신 사업보고서를 가져와줘 (DART)"
