...
class LegalComplianceAgent(DartBaseAgent):
    ...
    def __init__(self, model: str = "qwen-235b"):
        """LegalComplianceAgent 초기화 (Agent Portal 구조)"""
        super().__init__(
        ...
        self.agent_domain = "legal_risk"
        self.message_refiner = MessageRefiner()
        
        log_step("LegalComplianceAgent 초기화", "SUCCESS", "법적 리스크 분석 에이전트 설정 완료")

    async def _filter_tools(self, tools: List[MCPTool]) -> List[MCPTool]:
        """법적 리스크 분석에서 사용할 도구 필터링"""
        target_tools = {
            "get_default_notice",
            "get_rehabilitation_notice",
            "get_bankruptcy_notice",
            "get_lawsuit_notice",
            "get_administrative_action",
            "get_audit_opinion",
            "get_delisting_notice",
        }
        filtered = [t for t in tools if t.name in target_tools]
        log_step("도구 필터링 완료", "SUCCESS", f"LegalCompliance 도구: {len(filtered)}개")
        return filtered
    
    def _create_system_prompt(self) -> str:
        """시스템 프롬프트 생성"""
        return """당신은 DART 공시 시스템의 법적 리스크 분석 전문가입니다.
기업의 부도, 소송, 회생절차 등 법적 위험을 분석합니다."""

    async def _filter_tools_for_agent(self, tools):
        """LegalComplianceAgent에서 사용할 도구 필터링 (7개 도구)"""
        # README.md에 명시된 정확한 7개 법적 리스크 도구
        legal_tool_names = [
            # 경영위기 도구 (4개)
            "get_bankruptcy",  # 부도 발생 사실
            "get_business_suspension",  # 영업정지 사실
            "get_rehabilitation",  # 회생절차 개시신청 사실
            "get_dissolution",  # 해산사유 발생 사실
            # 채권관리 도구 (2개)
            "get_creditor_management",  # 채권은행 관리절차 개시 사실
            "get_creditor_management_termination",  # 채권은행 관리절차 종료 사실
            # 법적분쟁 도구 (1개)
            "get_lawsuit",  # 소송 제기 사실
        ]

        filtered_tools = []
        for tool in tools:
            if hasattr(tool, "name") and tool.name in legal_tool_names:
                filtered_tools.append(tool)

        log_step(
            "LegalComplianceAgent 도구 필터링",
            "SUCCESS",
            f"전체 도구: {len(tools)}개 → 법적 리스크 분석 도구: {len(filtered_tools)}개",
        )
    ...
    async def analyze_legal_risk_data(
        ...
        """법적 리스크 데이터 분석 메인 함수 - LLM 기반 도구 선택 + 스트리밍"""
        ...
        try:
            ...
            yield {
                "type": "progress",
                "content": f"{context.corp_name}의 법적 리스크 데이터를 수집하겠습니다...",
            }
                    ...
                    if agent_messages:
                        ...
                        if hasattr(agent_message, "content") and agent_message.content:
                            ...
                            if content:
                                yield {
                                    "type": "progress",
                                    "content": f"법적 리스크 분석 에이전트 분석: {content}...",
                                }
                                if final_response:
                                    final_response += "\n" + content
                                else:
            ...
            agent_result = AgentResult(
                ...
                analysis_type="legal_risk_streaming_analysis",
                risk_level=RiskLevel.LOW,
                key_findings=(
                    [final_response] if final_response else ["법적 리스크 데이터 분석 완료"]
                ),
                supporting_data={
                    "llm_response": final_response,
                    "raw_legal_data": collected_data,
            ...
            log_step("법적 리스크 분석 오류", "ERROR", f"분석 중 오류 발생: {str(e)}")
            import traceback

            log_step("법적 리스크 분석 오류", "ERROR", f"스택 트레이스: {traceback.format_exc()}")

            # 에러 yield
            yield {"type": "error", "content": f"법적 리스크 분석 중 오류가 발생했습니다: {str(e)}"}
