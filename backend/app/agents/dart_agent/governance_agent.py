...
class GovernanceAgent(DartBaseAgent):
    ...
    def __init__(self, model: str = "qwen-235b"):
        """GovernanceAgent 초기화 (Agent Portal 구조)"""
        super().__init__(
        ...
        self.agent_domain = "governance"
        self.message_refiner = MessageRefiner()
        
        log_step("GovernanceAgent 초기화", "SUCCESS", "지배구조 분석 에이전트 설정 완료")

    async def _filter_tools(self, tools: List[MCPTool]) -> List[MCPTool]:
        """지배구조 분석에서 사용할 도구 필터링"""
        target_tools = {
            ...
            "get_executive_info",
            "get_outside_director",
            "get_independent_auditor",
        }
        filtered = [t for t in tools if t.name in target_tools]
        log_step("도구 필터링 완료", "SUCCESS", f"Governance 도구: {len(filtered)}개")
        return filtered
    
    def _create_system_prompt(self) -> str:
        """시스템 프롬프트 생성"""
        return """당신은 DART 공시 시스템의 지배구조 분석 전문가입니다.
기업의 주주 구성, 임원 현황, 사외이사, 감사 등 지배구조를 분석합니다."""

    async def _filter_tools_for_agent(self, tools: List[BaseTool]) -> List[BaseTool]:
        """지배구조 분석에 특화된 도구 필터링 (8개 도구)"""
        # README.md에 명시된 정확한 8개 지배구조 도구
        ...
        log_step(
            "GovernanceAgent 도구 필터링",
            "SUCCESS",
            f"전체 도구: {len(tools)}개 → 지배구조 분석 도구: {len(filtered_tools)}개",
        )
    ...
    async def analyze_governance_data(
        ...
        """지배구조 분석 메인 함수 - LLM 기반 도구 선택 + 스트리밍"""
        ...
        try:
            ...
            yield {
                "type": "progress",
                "content": f"{context.corp_name}의 지배구조 데이터를 수집하겠습니다...",
            }
            ...
            async for chunk in self.agent_executor.astream(
                ...
                config={
                    "configurable": {
                        "thread_id": f"governance_{context.corp_code}_{int(time.time())}"
                    }
                },
            ):
                    ...
                    if agent_messages:
                        ...
                        if hasattr(agent_message, "content") and agent_message.content:
                            ...
                            if content:
                                yield {
                                    "type": "progress",
                                    "content": f"지배구조 분석 에이전트 분석: {content}...",
                                }
                                if final_response:
                                    final_response += "\n" + content
                                else:
            ...
            agent_result = AgentResult(
                ...
                analysis_type="governance_streaming_analysis",
                risk_level=RiskLevel.LOW,
                key_findings=[final_response] if final_response else ["지배구조 데이터 분석 완료"],
                supporting_data={
                    "llm_response": final_response,
                    "raw_governance_data": collected_data,
            ...
            log_step("지배구조 분석 오류", "ERROR", f"분석 중 오류 발생: {str(e)}")
            import traceback

            log_step("지배구조 분석 오류", "ERROR", f"스택 트레이스: {traceback.format_exc()}")

            # 에러 yield
            yield {"type": "error", "content": f"지배구조 분석 중 오류가 발생했습니다: {str(e)}"}
