...
from agent.dart_agent.utils.prompt_templates import PromptBuilder, get_business_structure_tools_description
...
class BusinessStructureAgent(BaseAgent):
    ...
    async def _filter_tools_for_agent(self, tools):
        ...
        # 사업구조 변화 관련 도구만 필터링 (README.md 기준 17개)
        target_tools = {
        ...
        for tool in tools:
            ...
            if tool_name in target_tools:
                ...
                log_step(
                    "도구 필터링",
                    "SUCCESS",
                    f"BusinessStructure 도구 추가: {tool_name}",
                )

        log_step(
            "도구 필터링 완료",
            "SUCCESS",
            f"BusinessStructureAgent에서 사용할 도구: {len(filtered_tools)}개",
        )
    ...
    async def analyze_business_structure_data(
        ...
        """사업구조 변화 데이터 분석 메인 함수 - LLM 기반 도구 선택 + 스트리밍"""
        ...
        try:
            ...
            yield {
                "type": "progress",
                "content": f"{context.corp_name}의 사업구조 변화 데이터를 수집하겠습니다...",
            }
            ...
            async for chunk in self.agent_executor.astream(
                ...
                config={
                    "configurable": {
                        "thread_id": f"business_{context.corp_code}_{int(time.time())}"
                    }
                },
            ):
                ...
                if "agent" in chunk:
                    ...
                    if agent_messages:
                        ...
                        if hasattr(agent_message, "content") and agent_message.content:
                            ...
                            if content:
                                yield {
                                    "type": "progress",
                                    "content": f"사업구조 변화 분석 에이전트 분석: {content}...",
                                }
                                if final_response:
                                    final_response += "\n" + content
                                else:
                    ...
                    for tool_message in tool_messages:
                        ...
                        if hasattr(tool_message, "content"):
                            ...
                            # get_investment_in_other_corp 도구는 통계로 변환
                            if tool_name == "get_investment_in_other_corp":
                                ...
                                collected_data[tool_name] = statistics_text  # LLM에 통계 데이터 전달
                                yield {
                                    "type": "progress",
            ...
            agent_result = AgentResult(
                ...
                analysis_type="business_structure_streaming_analysis",
                risk_level=RiskLevel.LOW,
                key_findings=(
                    [final_response] if final_response else ["사업구조 변화 데이터 분석 완료"]
                ),
                supporting_data={
                    "llm_response": final_response,
                    "raw_business_data": collected_data,
            ...
            log_step("사업구조 변화 분석 오류", "ERROR", f"분석 중 오류 발생: {str(e)}")
            import traceback

            log_step("사업구조 변화 분석 오류", "ERROR", f"스택 트레이스: {traceback.format_exc()}")

            # 에러 yield
            yield {
                "type": "error",
                "content": f"사업구조 변화 분석 중 오류가 발생했습니다: {str(e)}",
            }
