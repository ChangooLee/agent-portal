"""
부채 및 자금조달 분석 전문 에이전트 (6️⃣ 부채 및 자금조달 분석 도구들)
"""
...
class DebtFundingAgent(DartBaseAgent):
    ...
    def __init__(self, model: str = "qwen-235b"):
        """DebtFundingAgent 초기화 (Agent Portal 구조)"""
        super().__init__(
        ...
        self.agent_domain = "debt_funding"
        self.message_refiner = MessageRefiner()
        
        log_step("DebtFundingAgent 초기화", "SUCCESS", "부채 분석 에이전트 설정 완료")

    async def _filter_tools(self, tools: List[MCPTool]) -> List[MCPTool]:
        """부채 및 자금조달 분석에서 사용할 도구 필터링"""
        target_tools = {
            "get_debt",
        ...
        log_step("도구 필터링 완료", "SUCCESS", f"DebtFunding 도구: {len(filtered)}개")
        return filtered
    
    def _create_system_prompt(self) -> str:
        """시스템 프롬프트 생성"""
        return """당신은 DART 공시 시스템의 부채 및 자금조달 분석 전문가입니다.
기업의 채무증권 발행, 차입금, 부채 구조 등을 분석합니다."""

    async def _filter_tools_for_agent(self, tools: List[BaseTool]) -> List[BaseTool]:
        """부채 및 자금조달 분석에 특화된 도구 필터링 - README.md 기준 15개 도구"""
        # README.md에 명시된 정확한 15개 부채 및 자금조달 도구
        debt_funding_tool_names = [
            # 채무증권 발행 (6개) - README.md 기준
        ...
        filtered_tools = []
        for tool in tools:
            if hasattr(tool, "name") and tool.name in debt_funding_tool_names:
                filtered_tools.append(tool)

        log_step(
            "DebtFundingAgent 도구 필터링",
            "SUCCESS",
            f"전체 도구: {len(tools)}개 → 부채 및 자금조달 분석 도구: {len(filtered_tools)}개",
        )
    ...
    async def analyze_debt_funding_data(
        ...
        """부채 및 자금조달 분석 메인 함수 - LLM 기반 도구 선택 + 스트리밍"""
        ...
        try:
            ...
            yield {
                "type": "progress",
                "content": f"{context.corp_name}의 부채 및 자금조달 데이터를 수집하겠습니다...",
            }
            ...
            async for chunk in self.agent_executor.astream(
                ...
                config={
                    "configurable": {
                        "thread_id": f"debt_funding_{context.corp_code}_{int(time.time())}"
                    }
                },
            ):
                ...
                if "agent" in chunk:
                    ...
                    if agent_messages:
                        ...
                        if hasattr(agent_message, "content") and agent_message.content:
                            ...
                            if content:
                                yield {
                                    "type": "progress",
                                    "content": f"부채 및 자금조달 분석 에이전트 분석: {content}...",
                                }
                                if final_response:
                                    final_response += "\n" + content
                                else:
                        ...
                        if (
                            hasattr(agent_message, "tool_calls")
                    ...
                    for tool_message in tool_messages:
                        ...
                        if hasattr(tool_message, "content"):
                            ...
                            # get_debt와 get_debt_securities_issued 도구는 통계로 변환
                            if tool_name in ["get_debt", "get_debt_securities_issued"]:
                                from agent.dart_agent.dart_transformer import transform_dart_result
                                statistics_text = transform_dart_result(tool_name, extracted_text)
                                collected_data[tool_name] = statistics_text
                                yield {
                                    "type": "progress",
                                    "content": f"{display_name} 분석 결과:\n{statistics_text}",
                                }
                            else:
                                # 다른 도구는 기존 방식 유지
            ...
            agent_result = AgentResult(
                ...
                analysis_type="debt_funding_streaming_analysis",
                risk_level=RiskLevel.LOW,
                key_findings=(
                    [final_response]
                    if final_response
                    else ["부채 및 자금조달 데이터 분석 완료"]
                ),
                supporting_data={
                    "llm_response": final_response,
                    "raw_debt_funding_data": collected_data,
            ...
            log_step(
                "부채 및 자금조달 분석 오류", "ERROR", f"분석 중 오류 발생: {str(e)}"
            )
            import traceback
            ...
            yield {
                "type": "error",
                "content": f"부채 및 자금조달 분석 중 오류가 발생했습니다: {str(e)}",
            }
