"""
OTEL Metrics and Spans for RealEstate Agent

OpenTelemetry 기반 trace + metrics 헬퍼 함수들.
dart_agent/metrics.py 패턴 참조.
"""
...
_tracer = None
_service_tracers = {}
_active_service_name = "agent-realestate"
...
def _get_tracer():
    ...
    try:
        ...
        tracer = get_tracer_for_service(_active_service_name, "realestate-agent")
...
@contextmanager
def start_realestate_span(name: str, attributes: Optional[Dict[str, Any]] = None):
    """RealEstate 에이전트용 span 시작"""
    tracer = _get_tracer()
    
    span_attributes = {
        "agent.type": "realestate",
...
def record_tool_call(tool_name: str, arguments: Dict[str, Any], result: Any, latency_ms: float):
    """도구 호출 기록"""
    logger.info(f"[RealEstate Tool] {tool_name}: latency={latency_ms:.1f}ms, args={arguments}")
