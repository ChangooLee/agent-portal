"""
OTEL Metrics and Spans for Health Agent
...
_tracer = None
_service_tracers = {}
_active_service_name = "agent-health"
...
def _get_tracer():
    ...
    try:
        ...
        tracer = get_tracer_for_service(_active_service_name, "health-agent")
...
@contextmanager
def start_health_span(name: str, attributes: Optional[Dict[str, Any]] = None):
    """Health 에이전트용 span 시작"""
    tracer = _get_tracer()
...
def record_tool_call(tool_name: str, arguments: Dict[str, Any], result: Any, latency_ms: float):
    """도구 호출 기록"""
    logger.info(f"[Health Tool] {tool_name}: latency={latency_ms:.1f}ms, args={arguments}")
