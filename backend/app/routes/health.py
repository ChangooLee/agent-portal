"""
Health Agent API Routes

건강/의료 분석 에이전트 API 엔드포인트.
SSE 스트리밍 지원.
"""
...
router = APIRouter(prefix="/health-agent", tags=["Health Analysis"])
api_router = APIRouter(prefix="/api/health-agent", tags=["Health Analysis"])
...
@router.get("/status", summary="Health Agent Status Check")
@api_router.get("/status", summary="Health Agent Status Check")
async def health_check():
    ...
    try:
        ...
        logger.error(f"Health agent status check failed: {e}")
        return {
            "status": "error",
            "service": "health-agent",
            "mcp_connected": False,
...
@router.post("/chat/single", summary="Health Single Agent Chat (Streaming)")
@api_router.post("/chat/single", summary="Health Single Agent Chat (Streaming)")
async def chat_single_stream(request_data: HealthChatRequest):
    """
    건강/의료 분석 Single Agent 채팅 (SSE 스트리밍).
    
    Single Agent가 MCP 도구를 직접 사용하여 의료 정보를 분석합니다.
    """
    from app.agents.health_agent.single_agent import HealthSingleAgent
    
    logger.info(f"Health stream request: {request_data.question[:50]}...")
...
@router.post("/chat", response_model=HealthChatResponse, summary="Health Analysis Chat")
@api_router.post("/chat", response_model=HealthChatResponse, summary="Health Analysis Chat")
async def chat(request_data: HealthChatRequest):
    """
    건강/의료 분석 채팅 (동기 응답).
    ...
    logger.info(f"Health chat request: {request_data.question[:50]}...")
    ...
    try:
        ...
        logger.error(f"Health chat error: {e}", exc_info=True)
...
from app.services.agent_history_service import health_history_service
...
async def list_history(
    ...
    try:
        histories = await health_history_service.list_history(user_id, limit, offset)
...
async def search_history(
    ...
    try:
        histories = await health_history_service.search_history(user_id, query, limit)
        return {"success": True, "histories": histories, "count": len(histories)}
    except Exception as e:
...
async def get_history(
    ...
    try:
        history = await health_history_service.get_history(history_id, user_id)
        if not history:
...
async def create_history(
    ...
    try:
        history = await health_history_service.create_history(
            user_id=user_id,
            title=request.title,
...
async def update_history(
    ...
    try:
        success = await health_history_service.update_history(
...
async def delete_history(
    ...
    try:
        success = await health_history_service.delete_history(history_id, user_id)
        if not success:
            raise HTTPException(status_code=404, detail="History not found")
        return {"success": True}
    except HTTPException:
