...
async def get_usage_summary(project_id: Optional[str] = Query(None)):
    ...
    try:
        ...
        return {"langfuse": langfuse_summary, "helicone": helicone_summary, "status": "ok"}
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                return {"total_requests": 0, "total_tokens": 0, "total_cost": 0.0, "error": "Helicone API not available"}
    except Exception as e:
        return {"total_requests": 0, "total_tokens": 0, "total_cost": 0.0, "error": str(e)}
...
async def get_models():
    ...
    try:
        ...
        return {"models": models_data.get("data", []), "status": "ok"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def observability_health():
    ...
    checks = {"litellm": False, "langfuse": False, "helicone": False}
    ...
    return {"status": "ok" if any(checks.values()) else "degraded", "services": checks}
