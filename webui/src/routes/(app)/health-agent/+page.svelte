<script lang="ts">
	...
	async function loadHistories() {
		try {
			const response = await fetch('/api/health-agent/history');
			if (response.ok) {
	...
	async function searchHistories() {
		...
		try {
			const response = await fetch(`/api/health-agent/history/search?query=${encodeURIComponent(historySearchQuery)}`);
			if (response.ok) {
				const data = await response.json();
				histories = data.histories || [];
			}
	...
	async function saveHistory() {
		...
		try {
			if (currentHistoryId) {
				await fetch(`/api/health-agent/history/${currentHistoryId}`, {
					method: 'PUT',
	...
	async function loadHistory(historyId: string) {
		loadingHistory = true;
		try {
			const response = await fetch(`/api/health-agent/history/${historyId}`);
			if (response.ok) {
				const data = await response.json();
...
<svelte:head>
	<title>건강/의료 분석 | Health Agent</title>
</svelte:head>
		...
		<div class="relative px-6 py-6">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-4">
					<!-- 히스토리 버튼 -->
					<button 
						class="flex items-center gap-2 text-xs px-3 py-1.5 rounded-full transition-all {showHistorySidebar ? 'bg-red-500/20 text-red-300 border border-red-500/30' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 border border-gray-700/50'}"
					...
					{:else if mcpStatus === 'connected'}
						<div class="flex items-center gap-2 text-xs text-red-400 px-3 py-1.5 rounded-full bg-red-500/20 border border-red-500/30">
	...
	<div class="px-6 py-2 border-b border-gray-800/50 bg-gray-900/40 flex-shrink-0">
		<div class="flex items-center justify-end gap-2">
			...
			<select 
				...
				class="px-3 py-1.5 text-sm rounded-lg bg-gray-800/60 border border-gray-700/50 text-white focus:outline-none focus:ring-1 focus:ring-red-500/50 disabled:opacity-50"
			>
				{#if loadingModels}
		...
		<div class="flex-1 flex flex-col border-r border-gray-800/50 h-full relative" style="max-width: {showHistorySidebar ? 'calc(50% - 144px)' : '50%'}">
			...
			<div class="px-4 py-3 border-b border-gray-800/50 bg-gray-900/60 backdrop-blur-sm">
				<div class="flex items-center gap-2 text-sm text-gray-300">
					...
					{#if isLoading}
						<span class="ml-auto flex items-center gap-1.5 text-red-400">
				...
				{#each messages as message (message.id)}
					...
					>
						...
						{:else if message.role === 'user'}
							<div class="max-w-[85%] px-4 py-2.5 rounded-2xl bg-gradient-to-r from-red-500 to-rose-500 text-white text-sm shadow-md shadow-red-500/20">
				...
				<form on:submit|preventDefault={sendMessage} class="flex items-end gap-2">
					<div class="flex-1 relative">
						<textarea
							...
							placeholder="건강/의료 정보에 대해 질문하세요..."
							rows="1"
							disabled={isLoading}
							class="w-full resize-none rounded-xl border border-gray-700/50 bg-gray-800/60 backdrop-blur-sm px-4 py-2.5 text-sm text-white placeholder-gray-400 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all disabled:opacity-50"
					...
					<button
						...
						class="flex-shrink-0 p-2.5 rounded-xl bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-md shadow-red-500/20 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
					>
						{#if isLoading}
			...
			<div bind:this={reportContainer} class="flex-1 overflow-y-auto bg-gray-950">
				{#if report && (report.summary || report.sections.length > 0)}
					<div class="p-6 space-y-6" in:fade={{ duration: 300 }}>
						...
						<div class="flex items-center gap-3 pb-4 border-b border-gray-800/50">
							...
							<div>
								<h2 class="text-xl font-bold text-white">
									{report.title || '건강/의료 분석 결과'}
								</h2>
								<p class="text-sm text-gray-400">Health Analysis Report</p>
							</div>
						</div>
						...
						{#each report.sections as section, i}
							<div class="space-y-3" in:fly={{ y: 20, duration: 300, delay: i * 100 }}>
								<h3 class="flex items-center gap-2 text-lg font-semibold text-white">
									<span class="w-6 h-6 rounded-md bg-red-500/20 text-red-400 flex items-center justify-center text-sm font-bold border border-red-500/30">
										{i + 1}
									</span>
									{section.title}
								</h3>
								<div class="pl-8">
									<article class="prose prose-sm max-w-none prose-invert prose-headings:text-white prose-p:text-slate-300 prose-strong:text-white prose-code:text-slate-300 prose-pre:text-slate-200 prose-blockquote:text-slate-300 prose-li:text-slate-300 prose-a:text-red-400 
									...
									prose-tr:bg-gray-900/50 prose-tr:even:bg-gray-800/50">
										<Markdown id={`health-report-${i}`} content={section.content.trim()} />
									</article>
								</div>
							</div>
						{/each}
						...
						<!-- 스트리밍 중 표시 -->
						{#if reportStreaming}
							<div class="flex items-center gap-2 text-sm text-red-400 pl-8" in:fade>
								<div class="flex space-x-1">
									<div class="w-2 h-2 rounded-full bg-red-500 animate-bounce" style="animation-delay: 0ms"></div>
									<div class="w-2 h-2 rounded-full bg-red-500 animate-bounce" style="animation-delay: 150ms"></div>
									<div class="w-2 h-2 rounded-full bg-red-500 animate-bounce" style="animation-delay: 300ms"></div>
								</div>
								<span>레포트 생성 중...</span>
							</div>
						{/if}
					...
					<div class="flex-1 flex items-center justify-center h-full">
						<div class="text-center px-6 py-12">
							...
							<p class="text-sm text-gray-400 max-w-sm mx-auto">
								좌측에서 건강/의료 정보에 대해 질문하시면, AI가 분석한 결과가 구조화된 레포트로 나타납니다.
							</p>
						</div>
					</div>
				{/if}
