	...
	const setupSocket = async (enableWebsocket) => {
		try {
			...
			_socket.on('connect_error', (err) => {
				// Socket.IO 서버가 없을 수 있으므로 에러를 조용히 처리
			...
			// Socket.IO 서버가 있을 때만 연결 시도
			_socket.connect();
		} catch (error) {
			// Socket.IO 초기화 실패 시 조용히 처리
			console.warn('Socket.IO initialization failed (server may not support it):', error.message);
		}
	};
